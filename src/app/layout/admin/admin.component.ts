...
import { UserService } from 'C:/Users/yassi/Desktop/Projet geoInfo/guru-able-angular-lite-v8.0/default/src/app/Service/UserService';
...
export class AdminComponent implements OnInit {
  ...
  OnLogOut(){
    ...
    this.username=null;
    sessionStorage.setItem(this.USER_NAME_SESSION_ATTRIBUTE_NAME,null)
  }

  public config: any;
  USER_NAME_SESSION_ATTRIBUTE_NAME = 'authenticatedUser'
  username=null;
  constructor(public menuItems: MenuItems,private LoginService:LoginService,private userService:UserService) {
    //let a=this.UserService.username
  ...
  ngOnInit() {
    ...
    setTimeout(
      () => { 
        ...
        this.onGetRole()
        console.log((this.username=='null') +"         "+this.MonRole)
      }, 500
      );
    ...
    setTimeout(
      () => { 
        ...
        if(this.username!='null'){
          this.userService.GetRoleByEmail(this.username).then(
            (value) => {
              value["pipe"](map((res => res))).subscribe(result => {
                console.log(result);
                this.MonRole=result[0]['name']
                console.log("mon        role est :"+this.MonRole)
                })
            }
          );
        }
